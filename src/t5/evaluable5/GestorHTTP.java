...
public class GestorHTTP implements HttpHandler {
	...
	private void handlePostResponse(HttpExchange httpExchange, String requestParamValue)
		...
		if (requestParamValue.toString().split(":")[0].equals("notificarAveria")) {
			System.out.println("\nEntra a notifica averia");
			
				String email_remitente = requestParamValue.toString().split(":")[1].split("=")[1].split(";")[0];
				System.out.println(email_remitente);
				String pass_remitente = requestParamValue.toString().split(":")[1].split("=")[2];
			...
			try {
				...
				OutputStream outputStream = httpExchange.getResponseBody();
				String htmlResponse = "<html><body><h1>ENVIO REALIZADO CON EXITO</h1></body></html>";
				httpExchange.sendResponseHeaders(200, htmlResponse.length());
				outputStream.write(htmlResponse.getBytes());
				outputStream.flush();
				outputStream.close();
